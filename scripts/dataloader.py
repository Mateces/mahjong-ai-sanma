...
import libriichi; GameplayLoader = libriichi.dataset.GameplayLoader
        ...
        for file in data:
            for game in file:
                ...
                if game_size == 0 or len(at_kyoku) == 0:
                    ...
                    # chosen player. Skip cleanly instead of IndexError at
                    # `at_kyoku[-1]` below.
                    continue
                ...
                for i in range(game_size):
                    # Current 4-player scores, reordered to [self, +1, +2, +3]
                    # so feature index 0 is always "my score" — matches obs
                    # perspective. Used by score-prediction auxiliary loss.
                    _scores_abs = scores_seq[at_kyoku[i]]
                    _scores_rel = np.array([
                        _scores_abs[(player_id + j) % 4] for j in range(4)
                    ], dtype=np.float32)
                    entry = [
                        obs[i],
                        actions[i],
